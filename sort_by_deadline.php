...
<div class="container">
    	...
    	<ul>
			...
			$query = $pdo->query('SELECT * FROM `list` WHERE is_actual=1 ORDER BY `deadline` DESC');

			while($row = $query->fetch(PDO::FETCH_OBJ)) {
					echo '<div><li>'. $row->title .' <br> '. $row->text .'<br> Complete before '. $row->deadline .' '. $row->deadline_time .' <a href="set_as_closed.php?id='.$row->id.'" id="card-link-click" class="list" style="color:red"> х </a> <a href="set_as_completed.php?id='.$row->id.'" id="card-link-click" class="list" style="color:green"> ✓ </a></li>' . ' </div>';
